...
if (await page.getByLabel('Code', { exact: true }).isVisible().catch(() => false)) {
  if (!inviteCode) {
  ...
  await page.getByLabel('Code', { exact: true }).fill(inviteCode);
  await page.getByRole('button', { name: 'Continue' }).click();
  await page.getByLabel('Your name').waitFor({ timeout: 10000 });
